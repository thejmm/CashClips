...
async function handleSubscriptionChange(
  ...
  if (userDataError) {
    ...
    // Don't throw here, as we've already updated the subscription
  }
...
async function handleInvoicePayment(invoice: Stripe.Invoice, supabase: any) {
  ...
  const supabaseUserId = invoice?.metadata?.supabase_user_id;

  if (!supabaseUserId) {
    console.error("No Supabase user ID found in invoice metadata");
    return;
  }

  const invoiceData = {
    id: invoice.id, // Use Stripe's invoice ID as our primary key
  ...
  if (userDataError) {
    ...
    // Don't throw here, as we've already updated the invoice
  }
